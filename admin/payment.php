...
?></span><input type="button" id="transfer" value="碳幣回饋"><span id="message"></span>
<tr><td><label for="final_price">需付款金額<br />Final Price</label><td><span id="final_price"><?=$order_data['final_price']?></span>
<tr><td><label for="TransactionHash">帳單明細<br />Transaction Hash</label><td><span id="TransactionHash"><?=$order_data['TransactionHash']?><a href="<?=$order_data['TransactionHash']?>" target="_blank">連結</a></span>
...
<script>
var final = parseInt('<?php echo $order_data['coupon_price']; ?>');
