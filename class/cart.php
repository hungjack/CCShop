...
class Cart
{
	...
	//送出訂單
	function ConfirmOrder($order_id,$payment_method,$freight,$TransactionHash)
	{
		global $dbh;
		...
		$sql = "UPDATE `order` SET confirm_status=?,paid=?,payment_method=?,freight=?,TransactionHash=?,freight_price=?,order_date=CURRENT_TIMESTAMP,total_price=?,final_price=?-coupon_price WHERE order_id=?";
		$sth = $dbh->prepare($sql);
		$sth->execute([1,0,$payment_method,$freight,$TransactionHash,$freight_price,$freight_price+$cart['total_price'],$freight_price+$cart['total_price'],$order_id]);
	}
