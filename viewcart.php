		...
		if(!empty($_POST['name'])&&!empty($_POST['phone'])&&!empty($_POST['email'])&&
			!empty($_POST['address'])&&!empty($_POST['payment_method'])&&!empty($_POST['freight'])){
				if(!isset($order)) $msg = "操作無效";
					...
					$cart->ConfirmOrder($order->order_id,$_POST['payment_method'],$_POST['freight'],$_POST['TransactionHash']);
...
<tr><td>地址<td><input type="text" name="address" id="address" value="<?=$user->get_user_address();?>" required/>
<tr><td>付款方式
<td><label class="radio_label"><input name="payment_method" value="transfer" checked="checked" type="radio" required>轉帳匯款 </label><label class="radio_label"><input name="payment_method" value="visa" type="radio" required>信用卡</label><label class="radio_label"><input name="payment_method" value="pickup" type="radio" required>超商取貨付款</label>
<label class="radio_label"><input name="payment_method" value="CarbonCoin" type="radio" required>碳幣支付</label> <input type="button" id="transfer" value="支付結帳"></td>
<tr><td>交易明細<td><span id="message"></span><input type="hidden" id="TransactionHash" name="TransactionHash" value="" required>
<tr><td>取貨方式
<td><label class="radio_label"><input name="freight" value="mailing" checked="checked" type="radio" onclick="thumb_be_click(1)" required>郵寄($80)</label><label class="radio_label"><input name="freight" value="711" type="radio" onclick="thumb_be_click(2)" required>7-11取貨($60)</label>
<input type="hidden" id="pay" name="pay" value="1" /></td>
</tbody>
</table>
<div class="submit checkout"><input name="submit" id="submit" class="button" value="確認訂單！" type="submit"></div>
</form>
<?php }}
			else{
?>
...
<script>
var final = parseInt('<?php echo $final; ?>');
