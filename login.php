...
<?PHP
    ...
    {
        $account = $_POST['account']; //存取值
        $password = $_POST['password']; //存取值

        if (!empty($account) && !empty($password)) //存取值不能為空
        {
            $sql = "SELECT id, username, account_password FROM userinfo WHERE account = '$account'";  //SQL語法 
            $sth = $dbh->prepare($sql);
            $sth->execute();
